...
class TemplateProcessor(processor.ProcessorABC):
    ...
    def process(self, events):
        ...
        output["segment"].fill(mu_id=ak.flatten(segment.mu_id), 
        ...
        segment_slice_0 = segments_w_muon[segment_associated_muons.pt < 1]
        segment_slice_1 = segments_w_muon[(segment_associated_muons.pt > 1) & (segment_associated_muons.pt < 2)]
        segment_slice_2 = segments_w_muon[(segment_associated_muons.pt > 2) & (segment_associated_muons.pt < 5)]
        segment_slice_3 = segments_w_muon[(segment_associated_muons.pt > 5) & (segment_associated_muons.pt <10)]
        ...
        output["segment_slice_dxdz"].fill(
            pt_slice="$p_{T}$ > 10 GeV",
            slice_data=ak.flatten(segment_slice_4.dxdz)
            )
